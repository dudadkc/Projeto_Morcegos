...
names(covar_hydro) <- "hydro"
covar_cave <- terra::rast("01_data/01_variables/04_caves/01_adjusted/caves_neo.tif")
covar_edge <- terra::rast("01_data/01_variables/05_edge/forest_neo_distance.tif") %>%
    terra::project(crs(covar_cave)) %>%
    terra::resample(covar_cave, method = "near")
names(covar_edge) <- "edge"

range(values(covar_hydro), na.rm = TRUE)
range(values(covar_cave), na.rm = TRUE)
range(values(covar_edge), na.rm = TRUE)

covar <- c(covar_clim, covar_topo, covar_cave, covar_hydro, covar_edge)
covar
...
## export ----
path_sp <- paste0("02_results/03_01_sdms_v12")
